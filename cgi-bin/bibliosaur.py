...
class MyUser(db.Model):
  ...
  registeredemail = db.StringProperty()
  notificationwaittime = db.IntegerProperty()
  defaultformats = db.StringListProperty()
  defaultprice = db.IntegerProperty()
  preferredsort = db.StringProperty()
  ascending = db.BooleanProperty()
  labels = db.StringListProperty()
  def myput(self):
    self.put()
    key = "myuser" + self.registeredemail
    memcache.set(key, self)
    if (DEBUG >=10):
      logging.info("------- put myuser: " + self.registeredemail)
  def mydelete(self):
    key = "myuser" + self.registeredemail
    memcache.delete(key)
    if (DEBUG >=10):
...
class UserBook(db.Model):
  ...
  def myput(self):
    self.put()
    memcache.set("userbook" + self.goodreadsid + self.email, self)
    memcache.delete("displayxml" + self.goodreadsid + self.email, 600)
    memcache.delete("displaybook" + self.goodreadsid + self.email, 600)  
    if (DEBUG >=10):
      logging.info("------- put userbook: " + self.goodreadsid)
  def mydelete(self):
    memcache.delete("userbook" + self.goodreadsid + self.email)
    if (DEBUG >=10):
...
def GetMyUser(email):
  key = "myuser" + email
  myuser = memcache.get(key)
  if not myuser:
    myuser = MyUser.get_or_insert(parent=user_key(email), key_name=email)
    if not myuser.registeredemail:
      myuser.registeredemail = email
      myuser.myput()
    memcache.set(key, myuser)
  return myuser
...
def GetUserBook(goodreadsid, email):
  key = "userbook" + goodreadsid + email
  userbook = memcache.get(key)
  if not userbook:
    userbook = UserBook.get_or_insert(parent=user_key(email), key_name=goodreadsid)
    memcache.set(key, userbook, 3600)
  return userbook

def GetDisplayBookXML(userbook, myuser):
  key = "displayxml" + userbook.goodreadsid + userbook.email
  xmlbook = memcache.get(key)
  if not xmlbook:
	displaybook = GetDisplayBook(userbook, myuser)
	xmlbook = displaybook.xml().encode('utf-8')
	memcache.set(key, xmlbook)
  return xmlbook

def GetDisplayBook(userbook, myuser=None):
  key = "displaybook" + userbook.goodreadsid + userbook.email
  displaybook = memcache.get(key)
  if not displaybook:
	  ...
	  memcache.set(key, displaybook, 3600)
        ...
        for work in results.search.results.work:
          ...
          if goodreadsid:
            ...
            if (book.date < currenttime - timedelta) or (book.date > currenttime - timedeltatiny):
              ...
              book.myput()
              memcache.set("book" + goodreadsid, book)
            books.append(book)
      except AttributeError:
        pass
...
class ProduceDisplayBooksXML(webapp2.RequestHandler):
  def get(self):
	  ...
	  xmlfilecontent = open('getdisplaybooks.xml')
